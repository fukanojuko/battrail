...
「この見え方／手触りを変えたい」→「どこを触るか」の索引。数値は 2026-08 時点のもので、**正は常に
Inspector 側**（ここは古くなる前提で、場所とパラメータ名を見るために使う）。

仕様そのもの・なぜその設計かは [spec.md](spec.md) を参照。

---

## 触る前に

- **値はコードではなくシーン／プレハブ側で変える**。`[SerializeField]` の初期化子を書き換えても、
  既にシリアライズ済みのコンポーネントには反映されない（初期化子が使われるのは新規追加時だけ）。
  実際に `Combat.cs` の当たり判定を広げた変更が長期間効いていなかった事故がある
- **Play 中に変えた値は保存されない**。当たりを付けたらメモして、Play を止めてから同じ値を入れ直す
- コードの初期化子も同じ値に揃えておくと、次に誰かがコンポーネントを付け直したときにズレない
